...
import OutlookOAuthCallback from '../OutlookOAuthCallback';
...
function renderWith(searchParams: string) {
  ...
  return render(
    <QueryClientProvider client={qc}>
      <MemoryRouter initialEntries={[`/oauth/outlook/callback${searchParams}`]}>
...
describe('OutlookOAuthCallback', () => {
  it('shows error when oauth error param present', () => {
    renderWith('?error=access_denied');
    expect(screen.getByText('Outlook connection cancelled')).toBeInTheDocument();
  });
