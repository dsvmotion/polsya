...
import GoogleDriveOAuthCallback from '../GoogleDriveOAuthCallback';
...
function renderWith(searchParams: string) {
  ...
  return render(
    <QueryClientProvider client={qc}>
      <MemoryRouter initialEntries={[`/oauth/google-drive/callback${searchParams}`]}>
...
describe('GoogleDriveOAuthCallback', () => {
  it('shows error when oauth error param present', () => {
    ...
    expect(screen.getByText('Google Drive connection cancelled')).toBeInTheDocument();
  });
