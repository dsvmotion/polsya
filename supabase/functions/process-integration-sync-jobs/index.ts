...
type IntegrationHealthLevel = 'ok' | 'warning' | 'critical';
...
function parseLookbackHours(raw: unknown): number {
  return parsePositiveInt(raw, 24, 1, 168);
}

function parseStuckThresholdMinutes(raw: unknown): number {
  return parsePositiveInt(raw, 15, 1, 240);
}

function integrationMetricsSampleLimit(): number {
  return parsePositiveInt(Deno.env.get('INTEGRATION_METRICS_SAMPLE_LIMIT'), 2000, 100, 10_000);
}
...
function buildIntegrationHealthSummary(params: {
  jobs: readonly IntegrationHealthJobRow[];
  runs: readonly IntegrationHealthRunRow[];
  lookbackHours: number;
...
serve(async (req) => {
  ...
  try {
    ...
    if (body?.mode === 'metrics') {
      ...
      const sampleLimit = integrationMetricsSampleLimit();
