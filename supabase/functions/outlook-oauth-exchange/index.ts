...
serve(async (req) => {
  ...
  if (!clientId || !clientSecret || !redirectUri) {
    return jsonResponse({ error: 'Server misconfiguration: missing Outlook OAuth secrets' }, 500, corsHeaders);
  }
  ...
  try {
    ...
    if (!tokenRes.ok || !tokenJson.access_token) {
      ...
      return jsonResponse({ error: `Outlook token exchange failed: ${detail}` }, 400, corsHeaders);
    }
    ...
    try {
      ...
      if (meRes.ok) {
        ...
        providerEmail = (me.mail || me.userPrincipalName || null)?.toLowerCase() ?? null;
        if (typeof me.displayName === 'string' && me.displayName.trim().length > 0 && !metadata.sender_name) {
          metadata.sender_name = me.displayName.trim();
        }
      }
    } catch {
      // Best effort only.
